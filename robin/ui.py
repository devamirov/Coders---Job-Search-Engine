...
st.set_page_config(
    page_title="InfiNet Spectre — A Robin Dark Web OSINT Tool",
    page_icon="🕵️‍♂️",
    initial_sidebar_state="expanded",
)
st.markdown(THEME_CSS, unsafe_allow_html=True)

# Set custom favicon (robin.JPG) using base64 data URI
try:
    robin_favicon_path = os.path.join(os.path.dirname(__file__), "robin.JPG")
    if os.path.exists(robin_favicon_path):
        with open(robin_favicon_path, "rb") as f:
...
(function() {{
    ...
    link.href = 'data:image/jpeg;base64,{favicon_data}';
    document.head.appendChild(link);
    
    // Remove default Streamlit favicon
    const existingFavicons = document.querySelectorAll('link[rel="icon"]:not([href*="data:image"])');
...
'''
    ...
    pass  # Fail silently if favicon can't be loaded
...
if user is None:
    ...
    st.markdown("<h2 style='text-align:center;color:#8b6bab;'>InfiNet Spectre — A Robin Dark Web OSINT Tool</h2>", unsafe_allow_html=True)
...
if st.session_state.page == "profile":
    ...
    with logo_col:
        for path in (LOGO_INFINET_PATH, LOGO_PATH):
            try:
                st.image(path, width=180, use_container_width=False)
                break
            except Exception:
                continue
    st.markdown("---")
...
st.sidebar.metric("Searches used", f"{used} / {limit}")
...
if not st.session_state.whatsapp_verified:
    ...
    if not st.session_state.verify_code_sent:
        ...
        if st.session_state.get("verify_code_display"):
            err = st.session_state.get("verify_code_error") or "WhatsApp send failed"
            st.warning("**" + err + "** — Use this code to verify: **" + st.session_state.verify_code_display + "**")
