...
def _normalize_phone(phone: str) -> str:
    """Normalize for storage and comparison: digits and leading + only."""
...
def user_phone_taken(phone: str, exclude_uid: Optional[int] = None) -> bool:
    """True if this (normalized) phone is already linked to another user. One number = one account."""
    norm = _normalize_phone(phone)
    if not norm:
        return False
    with _conn() as c:
        if exclude_uid is not None:
            r = c.execute("SELECT id FROM users WHERE whatsapp = ? AND id != ?", (norm, exclude_uid)).fetchone()
        else:
            r = c.execute("SELECT id FROM users WHERE whatsapp = ?", (norm,)).fetchone()
    return r is not None


def user_set_whatsapp(uid: int, phone: str) -> bool:
    """Set user's WhatsApp number. Returns False if this number is already linked to another account."""
    norm = _normalize_phone(phone)
    if not norm:
        return False
    with _conn() as c:
        other = c.execute("SELECT id FROM users WHERE whatsapp = ? AND id != ?", (norm, uid)).fetchone()
        if other:
            return False
        c.execute("UPDATE users SET whatsapp = ? WHERE id = ?", (norm, uid))
    return True
...
def verification_code_create(user_id: int, phone: str) -> str:
    """Create a 6-digit code, store it (with normalized phone), return it."""
    norm = _normalize_phone(phone)
    if not norm:
    ...
    with _conn() as c:
        c.execute(
            ...
            (user_id, norm, code, expires.isoformat()),
        )
    return code


def verification_code_verify(user_id: int, phone: str, code: str) -> tuple[bool, Optional[str]]:
    """Verify code and link phone to user. Returns (True, None) on success, (False, reason) otherwise.
    reason: 'invalid_code' or 'phone_taken'. One number = one account."""
    now = datetime.now(timezone.utc).isoformat()
    norm = _normalize_phone(phone)
    code = (code or "").strip()
    if not norm:
